...
} from "@music-quiz/shared/schema/experience";
  ...
  add: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.tags && input.tags.length > 0) {
        const existingTags = await db.query.tagsTable.findMany({
          where: inArray(tagsTable.id, input.tags),
        });
        const existingTagIds = [...new Set(existingTags.map((tag) => tag.id))];

        const experienceTags = existingTagIds.map((tagId) => ({
          experienceId: experience.id,
          tagId,
          createdAt: new Date().toISOString(),
        }));
        await db.insert(experienceTagsTable).values(experienceTags);
      }
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      db.transaction(async (tx) => {
        ...
        if (input.tags && input.tags.length > 0) {
          ...
          await tx.insert(experienceTagsTable).values(experienceTags);
        }
      });
